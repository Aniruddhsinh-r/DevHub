...
<div class="space-y-0.5">
    ...
    @if ($type==='textarea')
        ...
        <textarea id="{{ $name }}" rows="4" name="{{ $name }}" value="{{ old($name,$value) }}" class="border border-gray-400 w-full p-2 font-medium text-sm text-gray-800 rounded-md shadow-xs focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500" placeholder="Write your thoughts here...">{{ old($name, $value) }}</textarea>
    @else
