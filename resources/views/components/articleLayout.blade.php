<div class="group relative bg-white rounded-[20px] overflow-hidden border border-gray-200 shadow-sm hover:shadow-2xl hover:-translate-y-1 transition-all duration-500 cursor-pointer">
    <a href="{{ route('articles.show', $article) }}" wire:navigate class="absolute inset-0 z-10" aria-label="{{ $article->title }}"></a>
    <div class="relative overflow-hidden h-52 bg-gray-300">
        ...
        @endif
        <div class="absolute top-4 right-4 bg-black/75 backdrop-blur-md text-white text-xs font-semibold px-4 py-2 rounded-full pointer-events-none">
    ...
    <div class="p-5">
        <div class="flex items-center gap-3 mb-3">
            ...
            @else
                <div class="w-9 h-9 rounded-full bg-black text-white flex items-center justify-center text-sm font-black uppercase shrink-0">
            ...
            <div class="text-right">
                @if(auth()->id() === $article->user_id)
                    <a href="{{ route('articles.edit', $article) }}" wire:navigate
                       class="relative z-20 inline-block bg-black/65 hover:bg-black/75 text-white font-semibold text-xs px-4 py-2 rounded-lg transition-all shadow-md">
                        Edit Article
                    </a>
                @else
