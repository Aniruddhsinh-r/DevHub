    ...
    <section class="max-w-3xl mx-auto px-4 py-10">
        <div class="flex items-center justify-between gap-4 mb-8">
            ...
            <form action="" method="GET" class="hidden sm:block">
                <div class="relative">
                    <input type="text" name="following" value="{{ request('search') }}" placeholder="Search following..." class="w-64 h-11 rounded-2xl border border-gray-200 bg-white pl-4 pr-11 text-sm font-medium text-black placeholder:text-gray-400 focus:outline-none focus:border-black transition-all duration-200">
        ...
        <div class="space-y-3">
            @forelse ($followings as $following)
                @if ($following->followed)
                    <a href="{{ route('userprofile',$following->followed->id) }}" class="block group bg-white border border-gray-200 hover:border-gray-400 rounded-2xl px-5 py-4 transition-all duration-300 hover:shadow-xl hover:-translate-y-1">
        ...
        <div class="mt-8">
            <a href="{{ route('userprofile',$user) }}" class="group inline-flex items-center gap-2 text-sm font-extrabold text-gray-500 hover:text-gray-800 transition-colors duration-150">
