...
<div class="min-h-screen bg-gray-200 flex flex-col justify-center items-center px-6 py-12">
    ...
    $homeRoute = $user && $user->hasAnyRole(['admin', 'superadmin'])
        ? 'filament.admin.pages.dashboard'
        : 'filament.app.pages.home';
@endphp

<a href="{{ route($homeRoute) }}"
    class="inline-flex justify-center items-center px-6 py-3 border border-transparent text-sm font-semibold rounded-xl text-white bg-indigo-600 hover:bg-indigo-700 shadow-lg shadow-indigo-100 transition-all duration-150">
    Go Back Home
</a>
    </div>
</div>
