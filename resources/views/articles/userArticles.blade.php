                    ...
                    @foreach($articles as $article)
                        <div class="group bg-white rounded-[20px] overflow-hidden border border-gray-200 shadow-sm hover:shadow-2xl hover:-translate-y-1 transition-all duration-500 relative">
                            ...
                            <div class="p-4 pt-3">
                                ...
                                <div class="mt-5 flex items-center justify-between border-t border-gray-200 pt-2">
                                    <div>
                                    ...
                                    <div class="flex flex-row items-end gap-1">
                                        <form action="/delete/{{ $article->id }}" method="POST" onsubmit="return confirm('Are you sure?')">@csrf @method('DELETE')<button class="px-4 py-2 rounded-xl bg-red-500 text-white hover:bg-red-600 transition-all duration-300 text-[11px] font-black tracking-wider">DELETE</button></form>

                                        <a href="/edit/{{ $article->id }}" class="px-4 py-2 rounded-xl bg-gray-900 text-white hover:bg-black transition-all duration-300 text-[11px] font-black tracking-wider">EDIT</a>
                                    </div>
                                </div>
                            </div>
                        </div>
