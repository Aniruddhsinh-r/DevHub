...
<main>
    ...
    <div class="flex overflow-hidden bg-[#f3f4f6] text-[#111827]" style="height: calc(100vh - 65px);">
        <aside class="w-14 sm:w-20 md:w-56 sticky top-16 bg-[#3b3b3d] border-r border-white/10 flex flex-col justify-between p-2 sm:p-4 transition-all duration-300 shrink-0">
            <div>
                <div class="hidden md:flex items-center gap-3 mb-5 border-b pb-4 border-white/10">
                    <div class="min-w-0 flex-1">
                        <span class="text-sm font-semibold text-white leading-none">{{ auth()->user()->name }}</span>
                        <span class="text-xs text-gray-200 block mt-0.5">{{ auth()->user()->email }}</span>
                    </div>
                </div>

                <nav class="flex flex-col gap-1">
                    <a href="{{ route('admin.dashboard') }}" wire:navigate class="flex items-center gap-3 p-2.5 rounded-lg cursor-pointer transition-all duration-200 md:justify-start sm:justify-center justify-center {{ request()->is('admin/dashboard') ? 'bg-white/10 text-white font-semibold shadow-sm' : 'text-gray-400 hover:bg-white/5 hover:text-white font-medium' }}">
                        <svg xmlns="http://www.w3.org/2000/svg" fill="currentColor" viewBox="0 0 24 24" class="w-5 h-5 {{ request()->is('admin/dashboard') ? 'text-white' : 'text-gray-400' }}">
                            <path d="M3 13h8V3H3v10zm0 8h8v-6H3v6zm10 0h8V11h-8v10zm0-18v6h8V3h-8z"/>
                        </svg>
                        <span class="text-sm md:block sm:hidden hidden">Dashboard</span>
                    </a>

                    <a href="{{ route('admin.users') }}" wire:navigate class="flex items-center gap-3 p-2.5 rounded-lg cursor-pointer transition-all duration-200 md:justify-start sm:justify-center justify-center {{ request()->is('admin/user*') ? 'bg-white/10 text-white font-semibold shadow-sm' : 'text-gray-400 hover:bg-white/5 hover:text-white font-medium' }}">
                        <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="2.5" stroke="currentColor" class="w-5 h-5 {{ request()->is('admin/user*') ? 'text-white' : 'text-gray-400' }}">
                    ...
                    <a href="{{ route('admin.articles') }}" wire:navigate class="flex items-center gap-3 p-2.5 rounded-lg cursor-pointer transition-all duration-200 md:justify-start sm:justify-center justify-center {{ request()->is('admin/article*') ? 'bg-white/10 text-white font-semibold shadow-sm' : 'text-gray-400 hover:bg-white/5 hover:text-white font-medium' }}">
                        <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="2.5" stroke="currentColor" class="w-5 h-5 {{ request()->is('admin/article*') ? 'text-white' : 'text-gray-400' }}">
                            <path stroke-linecap="round" stroke-linejoin="round" d="M12 6.042A8.967 8.967 0 0 0 6 3.75c-1.052 0-2.062.18-3 .512v14.25A8.987 8.987 0 0 1 6 18c2.305 0 4.408.867 6 2.292m0-14.25a8.966 8.966 0 0 1 6-2.292c1.052 0 2.062.18 3 .512v14.25A8.987 8.987 0 0 0 18 18a8.967 8.967 0 0 0-6 2.292m0-14.25v14.25" />
                        </svg>
                        <span class="text-sm md:block sm:hidden hidden">Articles</span>
                    </a>

                    <a href="{{ route('admin.categories') }}" wire:navigate class="flex items-center gap-3 p-2.5 rounded-lg cursor-pointer transition-all duration-200 md:justify-start sm:justify-center justify-center {{ request()->is('admin/categories') ? 'bg-white/10 text-white font-semibold shadow-sm' : 'text-gray-400 hover:bg-white/5 hover:text-white font-medium' }}">
                        <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="2.5" stroke="currentColor" class="w-5 h-5 {{ request()->is('admin/categories') ? 'text-white' : 'text-gray-400' }}">
                    ...
                    <a href="{{ route('admin.invitations') }}" wire:navigate class="flex items-center gap-3 p-2.5 rounded-lg cursor-pointer transition-all duration-200 md:justify-start sm:justify-center justify-center {{ request()->is('admin/invitations') ? 'bg-white/10 text-white font-semibold shadow-sm' : 'text-gray-400 hover:bg-white/5 hover:text-white font-medium' }}">
                        <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="2.5" stroke="currentColor" class="w-5 h-5 {{ request()->is('admin/invitations') ? 'text-white' : 'text-gray-400' }}">
