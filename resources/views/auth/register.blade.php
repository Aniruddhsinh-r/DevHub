            ...
            <div class="bg-white py-8 px-4 shadow-sm border border-gray-100 sm:rounded-2xl sm:px-10">
                <form method="POST" action="{{ route('register.store') }}" class="space-y-5" enctype="multipart/form-data">
                    ...
                    <div class="mt-8 flex items-center justify-between gap-4">
                        <a href="{{ route('login') }}" class="group inline-flex items-center gap-2 text-sm font-extrabold text-gray-500 hover:text-gray-800 transition-colors duration-150">
                        ...
                        <div>
                            <button type="submit" data-test="registerBTN" class="inline-flex justify-center py-2.5 px-6 border border-transparent rounded-xl shadow-sm text-sm font-semibold text-white bg-indigo-600 hover:bg-indigo-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-indigo-500 transition-all duration-200">
                                Register
                            </button>
                        </div>
                    </div>
